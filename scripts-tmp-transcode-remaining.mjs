...
// Only the 7 keys that failed last run (network drop) — the other 2 already succeeded.
const KEYS = [
...
const log = (msg) => {
  ...
  console.log(line)
  fs.appendFileSync(path.join(TMP_DIR, 'transcode2.log'), line + '\n')
}
...
async function withRetry(fn, label, attempts = 4) {
  ...
  for (let i = 1; i <= attempts; i++) {
    try {
      ...
      lastErr = err
      log(`  retry ${label}: attempt ${i}/${attempts} failed: ${err?.message ?? err}`)
...
async function main() {
  log(`=== Resume transcode job starting, ${KEYS.length} files ===`)
  const results = []
  ...
  log('=== Resume transcode job finished ===')
}
